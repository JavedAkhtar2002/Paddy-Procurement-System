...
contract PaddyProcurement {
    enum OrderStatus { PENDING ,QUALITY_ASSESSED ,ASSIGNED ,PAYMENT_COMPLETED ,IN_TRANSIT ,DELIVERED }
    ...
    function setTargetQuantity(uint256 _targetQuantity) external {
    ...
    function createOrder(address _fci, uint256 _quantity) external onlyFarmer{
        ...
        bytes32 orderId = keccak256(abi.encodePacked(msg.sender, _fci));
    ...
    function assessQuality(address _farmer, bytes32 _orderId) external onlyFCI {
    Order[] storage orders = ordersOfFarmer[_farmer];
    for (uint256 i = 0; i < orders.length; i++) {
    ...
    function completePayment(address _farmer, bytes32 _orderId, uint256 _quantity) external payable onlyFCI {
        Order[] storage orders = ordersOfFarmer[_farmer];
        uint256 amount = _quantity * 2;
        ...
        for (uint256 i = 0; i < orders.length; i++) {
            if (orders[i].orderId == _orderId && orders[i].status == OrderStatus.ASSIGNED) {
                ...
                emit PaymentCompleted(msg.sender, orders[i].farmer, amount, _orderId);
                ...
                payable(orders[i].farmer).transfer(amount);
                paymentMade = true;
                break;
            }
        }
    ...
    for (uint256 i = 0; i < orders.length; i++) {
        if (orders[i].orderId == _orderId && orders[i].status == OrderStatus.PAYMENT_COMPLETED) {
            require(block.timestamp <= orders[i].dateTime + 1 days, "Delivery start deadline passed");
    ...
    function getMillerCapacity(address _miller) external view returns (uint256, uint256, uint256) {
    MillerInventory memory inventory = millerInventories[_miller];
    return (inventory.maxStorageCapacity, inventory.targetQuantity, inventory.availableCapacity);
}
    ...
    // Create a new array to store accepted orders
    Order[] memory acceptedOrders = new Order[](acceptedCount);
    uint256 index = 0;

    // Populate the accepted orders array
    for (uint256 i = 0; i < allOrders.length; i++) {
        if (allOrders[i].assignedMiller == _miller && allOrders[i].status == OrderStatus.DELIVERED) {
            acceptedOrders[index] = allOrders[i];
            index++;
        }
    }
